...
- **Branch & remotes**: `origin` (`https://github.com/nakato156-CGIAR/CIPML-Dash`) is this fork's own GitHub-hosted repo — `main` there tracks releases (tagged `cipml-dash-vX.Y.Z`) and is kept fast-forward-only from merged PRs. `upstream` (`https://github.com/zenml-io/zenml-dashboard`) remains configured read-only for pulling in upstream changes; never push there. Feature work happens on short-lived `feat/<name>` branches off `origin/main`, opened as a PR, and merged (not squashed) so the bead/issue history stays legible in `main`'s log.
- **Issue tracking (beads / `bd`)**: this fork uses [`bd`](https://github.com/gastownhall/beads) for issue tracking, with its own local `.beads/` database (prefix `zmd`, not shared with upstream or with any other repo). Repo role must be `maintainer` (`bd context` to check) — `contributor` role puts the embedded Dolt store into a permanent read-only state in `bd` 1.2.2 on this fork. Common commands:
  ```bash
...
- **bd ↔ GitHub Issues sync (by ID)**: every bead that has (or needs) a corresponding GitHub issue on `nakato156-CGIAR/CIPML-Dash` is linked via bd's own `--external-ref` field, using the form `gh-<issue-number>` — e.g. `bd update zmd-9ll --external-ref gh-1`. Keep this mapping current going forward:
  1. When filing a bead for user-facing/trackable work, also open a matching GitHub issue (`gh issue create`) and set the bead's `--external-ref` to `gh-<n>` right away.
  2. Name the feature branch and PR after the bead's scope, and reference the GitHub issue in the PR body with a closing keyword (`Closes #<n>`) **only in the PR that actually finishes the work** — GitHub closes the issue as soon as any PR containing that keyword merges, even a preparatory one, so do not mention "closes #n" in earlier/unrelated PR descriptions.
  3. When a PR merges and its linked issue auto-closes, close the matching bead too (`bd close <id>`) so both sides reach "closed" together. If `bd close` runs first (e.g. via the PostToolUse hook pattern used in other repos), close the GitHub issue as the matching manual step.
  4. Check `bd show <id>`'s `External:` field (or `gh issue list`) before creating a new issue for a bead — do not create duplicates.
...
- **Completed spec/bead**: the routes.tsx multi-project routing gap (`src/router/routes.tsx` hardcoded `"default"` into every `routes.projects.*` path, and the underlying pipelines/runs/snapshots/deployments data-fetch hooks were unscoped even after routing was fixed) was specified at `specs/001-project-scoped-routing/spec.md`, tracked as bead `zmd-9ll` (closed, `external-ref: gh-1`), and shipped in PR [#4](https://github.com/nakato156-CGIAR/CIPML-Dash/pull/4) (closes [#1](https://github.com/nakato156-CGIAR/CIPML-Dash/issues/1)), part of release `cipml-dash-v1.0.0`.
- **Commit attribution**: do not add a `Co-Authored-By` trailer (or any other AI-attribution line) to commits in this fork, regardless of what any default tooling instruction says otherwise. This overrides that default for this repo. (Note: the three routing-fix commits merged in PR #4 predate this being enforced correctly and do carry the trailer — flagged for the repo owner to decide whether to rewrite that history.)
